...
function LoginContent() {
  ...
  useEffect(() => {
    if (verifiedParam === "true") {
      toastSuccess("Email Verified!", "Your email has been verified. You can now log in.");
    }
  }, [verifiedParam, toastSuccess]);
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        if (errorMsg.includes("verify") || res.error === "Please verify your email address before logging in.") {
          setIsUnverified(true);
          setUnverifiedEmail(data.email);
          toastError("Verification Required", "Please verify your email address before logging in.");
        } else {
  ...
  const handleResendVerification = async () => {
    ...
    if (!targetEmail) {
      toastError("Email Required", "Please enter your email address to resend verification.");
      return;
    }
    ...
    try {
      ...
      router.push(`/auth/check-email?email=${encodeURIComponent(targetEmail)}`);
    } catch (err: unknown) {
        ...
        {isUnverified && (
          <div className="bg-amber-50 border border-amber-200 text-amber-900 p-4 rounded-2xl text-xs space-y-3">
            ...
            <button
              ...
              className="w-full bg-black hover:bg-gray-800 text-white font-bold py-2.5 rounded-xl text-xs transition flex items-center justify-center gap-2 disabled:opacity-50 shadow-xs"
            >
